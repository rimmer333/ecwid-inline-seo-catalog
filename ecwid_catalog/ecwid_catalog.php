...
function show_ecwid($params) {
    ...
    $container_id = 'ecwid-product-browser-container';
    
    $views = array();
    if ($ecwid_pb_productspercolumn_grid > 0 && $ecwid_pb_productsperrow_grid > 0) {
        $views[] = "grid($ecwid_pb_productspercolumn_grid,$ecwid_pb_productsperrow_grid)";
    }
    if ($ecwid_pb_productsperpage_list > 0) {
        $views[] = "list($ecwid_pb_productsperpage_list)";
    }
    if ($ecwid_pb_productsperpage_table > 0) {
        $views[] = "table($ecwid_pb_productsperpage_table)";
    }
    $views = join(' ', $views);
...
<div id="$container_id">$html_catalog</div>
<script type="text/javascript"> 
    window.ecwid_loader();
    window.document.getElementById('$container_id').innerHTML = '';
